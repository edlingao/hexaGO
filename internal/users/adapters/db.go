...
func (s *Store[Item]) DeleteByField(field, value, table string) error {
  _, err := s.db.Exec("DELETE FROM " + table + " WHERE " + field + " = ?", value)

  return err
}

func (s *Store[Item]) GetByField(field, value, table string) ( Item, error ) {
  var item Item
...
func (s *Store[Item]) GetSQL(query string, item Item) ( Item, error ) {
